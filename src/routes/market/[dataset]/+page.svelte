<script lang="ts">
  ...
  import * as fixtures from "$lib/fixtures";
    ...
    const dataRows = lines.slice(1).map((line: string) => line.split(',').map((val: string) => parseFloat(val)));
    ...
    series: [
      {
        ...
        color: "#1A56DB",
        data: assetNames.map((name: string, i: number) => ({ x: name, y: currentAllocationData[i] })),
      },
      {
        name: "Proposed Allocation",
        color: "#FDBA8C",
        data: assetNames.map((name: string, i: number) => ({ x: name, y: proposedAllocationData[i] })),
      }
    ],
    chart: {
      type: "bar",
  ...
  $effect(() => {
    try {
      ...
      const series = assetNames.map((assetName: string, colIndex: number) => {
        return {
          name: assetName,
      ...
      if (lineChartOptions.xaxis) {
        lineChartOptions.xaxis.categories = dataRows.map((_: number[], index: number) => `Event ${index + 1}`);
      }
    } catch (error) {
  ...
  <Card class="h-full p-6">
    <form class="flex h-full flex-col justify-center">
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        <Button type="submit" size="xl" class="w-full" disabled={isInvalid} color="green">Buy</Button>
        <Button type="submit" size="xl" class="w-full" disabled={isInvalid} color="red">Sell</Button>
      </div>
      <Button type="button" size="xl" class="w-full" color="orange">Redeem</Button>
    </form>
  </Card>
